...
<head>
	...
	<title>Agent Login - NJInternational Travel Agency</title>
</head>
<body>
	<div class="limiter">
			...
			<div class="wrap-login100 animate__animated animate__fadeIn">
				...
				<form class="login100-form">
					<span class="login100-form-title">
						<h1 style="color:rgb(16, 148, 220)">AGENT LOGIN</h1>
					</span>

					<!-- Email or agent Code -->
					<div class="wrap-input100">
						<input class="input100" type="text" name="email_or_agent_code" placeholder="Email or agent Code" id="login_email_agent_code">
						<span class="focus-input100"></span>
						<span class="symbol-input100"><i class="fa fa-user" aria-hidden="true"></i></span>
					</div>
					<span class="text-danger login_email_agent_code_error"></span>
	...
	<script>
		...
		async function onLogin(event) {
			...
			document.querySelector('.login_email_agent_code_error').innerText = '';
			document.querySelector('.login_password_error').innerText = '';

			// Get inputs
			let email_or_agent_code = document.getElementById('login_email_agent_code').value.trim();
			let password = document.getElementById('password').value.trim();

			let isError = false;

			// Validation
			if (!email_or_agent_code) {
				document.querySelector('.login_email_agent_code_error').innerText = 'Input Field is required';
				isError = true;
			}
